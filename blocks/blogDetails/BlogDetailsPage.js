import { observerMixin } from "./Mixins.js";
import { Commnad } from "./SaveCommand.js"; 
import { COMMANDS } from "./SaveCommand.js";
import { CommandExecutor } from "./SaveCommand.js";

class BlogList {
    #blogs = [];
    #limitedView = true;

    get blogs() {
        if (this.#limitedView)
            return this.#blogs.slice(0, 4); 
        return this.#blogs;
    }

    get allBlogs() {
        return this.#blogs;
    }

    get limitedView() {
        return this.#limitedView;
    }

    static instance = null;
    static {
        this.instance = new BlogList();
    }

    constructor() {
        if (BlogList.instance && BlogList.instance !== this) { 
             throw new Error("Ya existe una instancia de BlogList");
        }
        
        if(!BlogList.instance){
            BlogList.instance = this;
        }
    }

    static getInstance() {
        if (!this.instance) {
            this.instance = new BlogList();
        }
        return this.instance;
    }

    find(id) {
        return this.#blogs.find(blog => blog.id == id);
    }

    addLike(id) {
        const blog = this.find(id);
        if (blog) {
            blog.likes = Number(blog.likes) + 1;
            blog.like = "true";
            this.notify();
        }
    }

    removeLike(id) {
        const blog = this.find(id);
        if (blog) {
            blog.likes = Number(blog.likes) - 1;
            blog.like = "false";
            this.notify();
        }
    }

    saveBlog(id) {
        const blog = this.find(id);
        if (blog) {
            blog.save = "true";
            this.notify();
            
            const command = new Commnad(COMMANDS.SAVE, blog); 
            CommandExecutor.execute(command);
        }
    }

    unsaveBlog(id) {
        const blog = this.find(id);
        if (blog) {
            blog.save = "false";
            
            const command = new Commnad(COMMANDS.UNSAVE, blog);
            CommandExecutor.execute(command);
            this.notify(); 
        }
    }

    addBlog(blog) {
        this.#blogs.push(blog);
        this.notify();
    }

    setBlogs(blogs) {
        this.#blogs = blogs;
        this.notify(); 
    }

    seeMore() {
        this.#limitedView = false;
        this.notify(); 
    }
}

Object.assign(BlogList.prototype, observerMixin);

export default BlogList;